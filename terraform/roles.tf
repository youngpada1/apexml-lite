...
resource "snowflake_account_role" "transformer" {
  ...
  comment = "dbt role — reads RAW, writes STAGING and ANALYTICS"
}

resource "snowflake_account_role" "reader" {
  name    = "APEXML_READER"
  comment = "Read-only role — Streamlit and Cortex AI access to ANALYTICS"
}
...
# ─────────────────────────────────────────────
# Schema grants — TRANSFORMER → RAW (read) + STAGING + ANALYTICS (write)
# ─────────────────────────────────────────────
...
resource "snowflake_grant_privileges_to_account_role" "transformer_analytics_schema" {
  ...
  privileges        = ["USAGE", "CREATE TABLE", "CREATE VIEW"]
  on_schema {
    schema_name = "\"${snowflake_database.apexml_db.name}\".\"${snowflake_schema.analytics.name}\""
  }
}

# ─────────────────────────────────────────────
# Schema grants — READER → ANALYTICS (read-only)
# ─────────────────────────────────────────────
...
resource "snowflake_grant_privileges_to_account_role" "reader_analytics_tables" {
  ...
  on_schema_object {
    all {
      object_type_plural = "TABLES"
      in_schema          = "\"${snowflake_database.apexml_db.name}\".\"${snowflake_schema.analytics.name}\""
    }
  }
}
