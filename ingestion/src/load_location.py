"""Load location for all sessions into Snowflake RAW."""
...
def fetch_drivers(session_key: int) -> list[int]:
    transport = httpx.HTTPTransport(retries=3)
    with httpx.Client(timeout=30.0, transport=transport) as client:
        response = client.get(f"{config.OPENF1_BASE_URL}/drivers", params={"session_key": session_key})
        response.raise_for_status()
        data = response.json()
    return [d["driver_number"] for d in data if "driver_number" in d]
...
def main():
    ...
    print(f"[{ts}] Starting load_location")
    ...
    for i, sk in enumerate(session_keys, 1):
        ...
        try:
            ...
            if not drivers:
                print("no drivers found, skipping.")
                skipped += 1
                continue

            session_rows = 0
            for driver_number in drivers:
                rows = fetch_location(sk, driver_number)
                for r in rows:
                    cur.execute(
            ...
            conn.commit()
            loaded += session_rows
            print(f"{session_rows} records inserted ({len(drivers)} drivers).")
