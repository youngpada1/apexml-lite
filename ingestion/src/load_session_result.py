"""Load session results for all sessions into Snowflake RAW."""
...
TABLE = "APEXML_DB.RAW.SESSION_RESULT"
...
def fetch_session_result(session_key: int) -> list[dict]:
    ...
    with httpx.Client(timeout=30.0, transport=transport) as client:
        response = client.get(
            f"{config.OPENF1_BASE_URL}/session_result",
...
def main():
    ...
    print(f"[{ts}] Starting load_session_result")
