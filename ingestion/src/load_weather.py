"""Load weather for all sessions into Snowflake RAW."""
...
def fetch_weather(session_key: int) -> list[dict]:
    ...
    with httpx.Client(timeout=30.0, transport=transport) as client:
        response = client.get(
            f"{config.OPENF1_BASE_URL}/weather",
...
def main():
    ...
    print(f"[{ts}] Starting load_weather")
