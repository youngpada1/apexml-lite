"""Load team radio for all sessions into Snowflake RAW."""
...
TABLE = "APEXML_DB.RAW.TEAM_RADIO"
...
def fetch_team_radio(session_key: int) -> list[dict]:
    ...
    with httpx.Client(timeout=30.0, transport=transport) as client:
        response = client.get(
            f"{config.OPENF1_BASE_URL}/team_radio",
...
def main():
    ...
    print(f"[{ts}] Starting load_team_radio")
