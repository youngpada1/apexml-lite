"""Load sessions into Snowflake RAW."""
...
def main():
    ...
    print(f"[{ts}] Starting load_sessions")

    rows = fetch_all_sessions()
    print(f"Fetched {len(rows)} sessions from API")
    ...
    for i, r in enumerate(rows, 1):
        ...
        print(f"[{i}/{len(rows)}] session_key={sk}...", end=" ", flush=True)
        try:
    ...
    print(f"\n[{ts_end}] Done. Loaded {loaded} sessions.")
